...
import { CheckCircle, Clock, AlertCircle, X, Bell, Check, Trash2 } from "lucide-react";
import {
...
const NotificationItem = ({ notification, onMarkAsRead, onDelete, onMarkAsUnread }) => {
  const getIcon = (type) => {
    switch (type) {
      case 'success':
...
const CustomerNotificationModal = ({ customerId, isOpen, onClose }) => {
  ...
  const loadNotifications = useCallback(async () => {
    if (!customerId) {
      console.warn('No customerId provided to CustomerNotificationModal');
      return;
    }
    try {
      setLoading(true);
      const response = await getNotifications('customer', customerId);
      if (response.success) {
  ...
  const loadUnreadCount = useCallback(async () => {
    ...
    try {
      const response = await getUnreadCount('customer', customerId);
      if (response.success) {
  ...
  }, [isOpen, customerId, loadNotifications, loadUnreadCount]);

  // Notification management functions
  const handleMarkAsRead = async (notificationId) => {
    try {
      await markAsRead(notificationId, 'customer', customerId);
  ...
  const handleMarkAsUnread = async (notificationId) => {
    try {
      await markAsUnread(notificationId, 'customer', customerId);
      setNotifications(prev =>
        prev.map(notification =>
          notification._id === notificationId
  ...
  const handleDeleteNotification = async (notificationId) => {
    try {
      await deleteNotification(notificationId, 'customer', customerId);
      setNotifications(prev =>
  ...
  const handleMarkAllAsRead = async () => {
    try {
      await markAllAsRead('customer', customerId);
      setNotifications(prev =>
  ...
  const handleClearAll = async () => {
    try {
      await clearAllNotifications('customer', customerId);
      setNotifications([]);
      setUnreadCount(0);
    } catch (error) {
