...
import { adminGetInvoice, adminCloseInvoice } from "../../api/invoices";
...
export default function InvoiceDetailAdmin() {
  ...
  useEffect(() => { load(); /* eslint-disable */ }, [id]);

  const onCloseInvoice = async () => {
    try {
      setBusy(true);
      await adminCloseInvoice(id);
      toast.success("Invoice closed.");
      await load();
    } catch (e) {
      toast.error(e?.response?.data?.message || "Failed to close invoice");
    } finally {
      setBusy(false);
    }
  };
  ...
  if (!row)   return <main style={page}><div style={muted}>Not found.</div></main>;

  return (
    <main style={page}>
      {/* Top header (visual only) */}
      <header style={head}>
        <div>
          <h2 style={h2}>Invoice <span style={mono}>#{row.invoiceNumber || row._id.slice(-6)}</span></h2>
          <div style={subline}>
        ...
        <div style={actions}>
          <Link to={`/admin/purchase-orders/${row.purchaseOrder}`} style={ghostBtn}>Open PO</Link>
          {row.status !== "closed" && (
            <button onClick={onCloseInvoice} disabled={busy} style={primaryBtn}>
              {busy ? "Closing…" : "Close Invoice"}
            </button>
          )}
          <Link to="/admin/invoices" style={ghostBtn}>Back</Link>
        </div>
      </header>
          ...
          <table style={tbl}>
            <thead style={thead}>
              <tr>
                ...
                <th style={{ ...th, textAlign: "right", width: 140 }}>Unit</th>
                <th style={{ ...th, textAlign: "right", width: 160 }}>Line</th>
              </tr>
            </thead>
            <tbody>
              {(row.items || []).map((it, i) => (
                <tr key={i} style={{ ...tr, ...(i % 2 ? rowAlt : null) }}>
                  ...
                  <td style={{ ...td, textAlign: "right", fontWeight: 700 }}>{money(it.lineTotal)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <>
            ...
            <ul style={{ marginTop: 6 }}>
              {row.additionalCharges.map((c, i) => (
                <li key={i}>{c.label}: <b>{money(c.amount)}</b></li>
              ))}
            </ul>
          </>
        )}
...
const head = {
  ...
  padding: 16, border: "1px solid #e2e8f0", borderRadius: 16,
...
const cardHead = { display: "flex", alignItems: "baseline", justifyContent: "space-between" };
...
const badge = { display: "inline-block", padding: "2px 10px", borderRadius: 999, fontSize: 12, fontWeight: 800, border: "1px solid transparent" };
...
const ghostBtn = {
  ...
  background: "#fff", color: "#0f172a", fontWeight: 700, textDecoration: "none", cursor: "pointer"
};
const primaryBtn = {
  padding: "10px 14px", borderRadius: 10, border: "1px solid #0284c7",
